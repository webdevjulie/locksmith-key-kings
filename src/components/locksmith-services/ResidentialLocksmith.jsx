...
export default function ResidentialLocksmith() {
  const residential = {
    title: "Residential Locksmith Services",
    subtitle: "Fast, reliable, and secure",
    backgroundImage: "../images/residential-locksmith.webp",
    description: `
      Fast and reliable residential locksmith services in South Bend. 
    ...
    form: {
      ...
      fields: [
        ...
        { type: "text", id: "address", name: "address", label: "Service Address *", required: true },
        {
          ...
          required: true,
          options: [
            "House Lockout", "Lock Replacement", "Lock Rekey", "Lock Repair",
            "Gate Locks", "Master Key Systems", "Emergency Lockouts"
          ]
        },
        {
          type: "select",
  ...
  const lockoutServices = {
    header: {
      title: "Our Services",
      subtitle: "Trusted residential locksmith solutions tailored to your needs.",
    },
    services: [
      {
        id: 1,
        title: "Lock Replacement",
        description: "Replace damaged or outdated locks quickly to ensure security.",
        image: "../images/lock-installation.webp",
      },
      {
        id: 2,
        title: "Lock Rekey",
        description: " Change the internal pins of existing locks to work with a new key.",
        image: "../images/rekey.webp",
      },
      {
        id: 3,
        title: "Lock Repair",
        description: "Fix broken or malfunctioning locks for enhanced safety.",
        image: "../images/lock-repair.webp",
      },
      {
        id: 4,
        title: "Gate Locks",
        description: "Secure gates with professional installation and repair services.",
        image: "../images/gate-locks.webp",
      },
    ],
  };
  ...
  const howItWorks = {
    header: {
      title: "How It Works",
      subtitle: "Our 4-step process ensures a stress-free locksmith experience.",
    },
    steps: [
      {
        id: 1,
        title: "Request Service",
        description: "Call us or book online with your service details and location.",
      },
      {
        id: 2,
        title: "Quick Dispatch",
        description: "We send the nearest locksmith to your location immediately.",
      },
      {
        id: 3,
        title: "Expert Service",
        description: "Our licensed locksmith solves the issue efficiently and securely.",
      },
      {
        id: 4,
        title: "Peace of Mind",
        description: "Enjoy restored security and access, with work guaranteed.",
      },
    ],
  };
  ...
  const lockoutTips = {
    header: {
      title: "Residential Security Tips",
      subtitle: "Protect your home and family with these practical tips.",
    },
    tips: [
      {
        id: 1,
        title: "Upgrade Old Locks",
        description: "Replace outdated locks with high-security deadbolts or smart locks.",
      },
      {
        id: 2,
        title: "Rekey After Moving",
        description: "Always rekey or replace locks when moving into a new home.",
      },
      {
        id: 3,
        title: "Spare Key Safety",
        description: "Avoid hiding spare keys outdoors—use a trusted neighbor instead.",
      },
      {
        id: 4,
        title: "Routine Maintenance",
        description: "Lubricate and inspect locks regularly to prevent failures.",
      },
    ],
  };
  ...
  return (
    <>
      ...
      <section
        ...
        style={{ backgroundImage: `url(${residential.backgroundImage})` }}
      >
        ...
        <div className="relative z-10 w-full max-w-7xl flex flex-col md:flex-row items-center justify-between px-6 lg:px-12 gap-12">
          ...
          <div className="flex-1 flex flex-col justify-center space-y-6 md:space-y-8 max-w-xl text-center md:text-left">
            ...
            <div className="flex flex-col sm:flex-row gap-4 mt-6 justify-center md:justify-start">
              <a
                href={`tel:+1${residential.phone.replace(/\D/g, "")}`}
                className="group flex items-center justify-center gap-2 bg-[#FD9A00] hover:bg-[#F58600] transition-all duration-300 rounded-2xl px-6 py-4 font-semibold text-white text-base transform hover:scale-105 shadow-xl hover:shadow-2xl min-w-[220px] relative overflow-hidden"
              >
                ...
                <span>Call {residential.phone}</span>
              </a>

              <a
                href="../book-service.html"
                className="group flex items-center justify-center gap-2 border-2 border-[#FD9A00] rounded-2xl px-6 py-4 font-semibold text-white text-base hover:bg-gradient-to-r hover:from-[#FD9A00] hover:to-[#F58600] hover:border-white transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-2xl min-w-[220px]"
              >
                <span>Book Now</span>
            ...
            <div className="flex flex-wrap gap-3 mt-6 justify-center md:justify-start">
              {residential.badges.map((badge, idx) => (
                <span
                  key={idx}
                  className="inline-flex items-center gap-2 bg-white/10 border border-white/20 rounded-full px-4 py-2 text-xs sm:text-sm shadow-lg backdrop-blur-md text-white"
                >
          ...
          {/* Right Side (Form) */}
          <div className="flex-1 relative z-10 bg-gradient-to-br from-white to-gray-50 rounded-3xl shadow-2xl p-8 sm:p-10 text-[#1C1C1C] w-full max-w-lg border border-gray-100">
            <h3 className="text-2xl sm:text-3xl font-extrabold mb-8 text-center text-[#1C1C1C]">
              {residential.form.heading}
            </h3>

            <form id="booking-form" className="space-y-6" noValidate>
              {/* Name & Phone */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {residential.form.fields.slice(0, 2).map((field, idx) => (
              ...
              <div className="relative">
                <input
                  type="text"
                  id={residential.form.fields[2].id}
                  name={residential.form.fields[2].name}
                  placeholder=" "
                  required
                  className="peer w-full border rounded-xl p-3 text-sm shadow-sm bg-transparent focus:ring-2 focus:ring-[#FD9A00] focus:border-[#FD9A00] focus:outline-none transition placeholder-transparent"
                />
                <label
                  htmlFor={residential.form.fields[2].id}
                  className="absolute left-3 top-3 text-gray-500 text-sm transition-all peer-placeholder-shown:top-3 peer-placeholder-shown:text-gray-400 peer-placeholder-shown:text-sm peer-focus:top-[-8px] peer-focus:text-xs peer-focus:text-[#FD9A00] bg-white px-1"
                >
                  {residential.form.fields[2].label}
                </label>
              </div>

              {/* Select Fields */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {residential.form.fields.slice(3, 5).map((field, idx) => (
                  <div key={idx} className="relative">
                    <select
                      id={field.id}
                      name={field.name}
              ...
              <div className="relative">
                <textarea
                  id={residential.form.fields[5].id}
                  name={residential.form.fields[5].name}
                  rows="3"
                  placeholder=" "
                  className="peer w-full border rounded-xl p-3 text-sm shadow-sm bg-transparent focus:ring-2 focus:ring-[#FD9A00] focus:border-[#FD9A00] focus:outline-none transition placeholder-transparent"
                ></textarea>
                <label
                  htmlFor={residential.form.fields[5].id}
                  className="absolute left-3 top-3 text-gray-500 text-sm transition-all peer-placeholder-shown:top-3 peer-placeholder-shown:text-gray-400 peer-placeholder-shown:text-sm peer-focus:top-[-8px] peer-focus:text-xs peer-focus:text-[#FD9A00] bg-white px-1"
                >
                  {residential.form.fields[5].label}
                </label>
              </div>
              ...
              <button
                type="submit"
                className="w-full py-4 bg-gradient-to-r from-[#FD9A00] to-[#F9AE19] text-white font-bold rounded-2xl shadow-lg hover:shadow-2xl hover:scale-[1.02] transition-all duration-300 flex items-center justify-center gap-2 text-lg tracking-wide"
              >
                <span className="material-icons text-xl animate-pulse">send</span>
                {residential.form.buttonText}
              </button>
            </form>
          </div>
        </div>
